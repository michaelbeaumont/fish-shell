...
impl Autoload {
    ...
    pub fn new(env_var_name: &'static wstr) -> Self {
        Self {
            env_var_name,
            autoloaded_files: HashMap::new(),
            current_autoloading: HashSet::new(),
            cache: Box::new(AutoloadFileCache::new()),
        }
    }
    ...
    pub fn perform_autoload(path: &wstr, parser: &Parser) {
        ...
        let parser = ScopeGuard::new(parser, |parser| parser.set_last_statuses(prev_statuses));
    ...
    pub fn get_autoloaded_commands(&self) -> Vec<WString> {
        let mut result = vec![];
        result.reserve(self.autoloaded_files.len());
