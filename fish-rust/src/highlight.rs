...
use crate::ast::{
    ...
    Leaf, List, Node, NodeFfi, NodeVisitor, Redirection, Token, Type, VariableAssignment,
};
...
use crate::env::{EnvDynFFI, EnvStackRefFFI, Environment};
use crate::expand::{
...
use crate::ffi::{self, highlight_spec_t, rgb_color_t};
use crate::function;
...
impl<'s> Highlighter<'s> {
    ...
    fn color_as_argument(&mut self, node: &dyn ast::Node, options_allowed: bool /* = true */) {
        ...
        ) > 0
        {
            ...
            let arg_cursor = self.cursor.map(|c| c - arg_subcmd_start);
